...
class SectorYieldDataTable extends StatefulWidget {
  final String sectorId;
  final String sectorName;

  const SectorYieldDataTable(
      {super.key, required this.sectorId, required this.sectorName});

  @override
  State<SectorYieldDataTable> createState() => _SectorYieldDataTableState();
}

class _SectorYieldDataTableState extends State<SectorYieldDataTable> {
  ...
  Future<void> _loadYieldData() async {
    ...
    try {
      ...
      final apiData =
          await sectorService.fetchSectorYieldData(sectorId: widget.sectorId);
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      padding: EdgeInsets.all(isWeb ? 0 : 0),
  ...
  Widget _buildAssociationHeader(ThemeData theme) {
    return Card(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                // Excel button (green)
                _buildExportButton(theme, isPDF: false),
                const SizedBox(height: 8),
                // PDF button (red)
                _buildExportButton(theme, isPDF: true),
              ],
            )
          ],
        ),
