 ...
 // ignore_for_file: avoid_types_as_parameter_names, non_constant_identifier_names, empty_catches
...
import 'package:flareline/pages/map/map_widget/map_panel/barangay_modal.dart';
import 'package:flareline/pages/map/map_widget/map_panel/lake_modal.dart';
import 'package:flareline/pages/map/map_widget/map_panel/brgy_prev.dart';
import 'package:flareline/pages/map/map_widget/map_panel/farm_creation/farm_creation_modal.dart';
import 'package:flareline/pages/map/map_widget/map_panel/farm_prev.dart';
import 'package:flareline/pages/map/map_widget/map_panel/lake_prev.dart';
...
// Simple update counter
  ...
  final ValueNotifier<int> editorUpdateNotifier = ValueNotifier<int>(0);
   ...
   final Map<int, GlobalKey> _weatherChipKeys = {};
  
  // Get or create a GlobalKey for a polygon's weather chip
  GlobalKey _getWeatherChipKey(int polygonId) {
    return _weatherChipKeys.putIfAbsent(
      polygonId,
      () => GlobalKey(),
    );
  }
          ...
          builder: (overlayContext) => Positioned(
            ...
            child: Align(
              ...
              child: InfoCard(
                ...
                left:mapWidgetPosition.dx , 
                width: mapWidgetSize.width,  
                weatherChipKey: _getWeatherChipKey(polygon.id!), // ADD THIS
                openWeatherApiKey: '0e0d32c1dcc47a3adc567ec7923c2508',
                showWeather: true,
                onTap: () {
                  removeInfoCardOverlay();
                  if (context.mounted) {
  ...
  void _showInfoCardFallback(
    ...
    try {
      ...
      width: 0,
                  ...
                  polygon: polygon,
                   openWeatherApiKey: '0e0d32c1dcc47a3adc567ec7923c2508',
                    weatherChipKey: _getWeatherChipKey(polygon.id!),
  showWeather: true, // Set to false to disable weather
                  onTap: () {
                    removeInfoCardOverlay();
                    if (context.mounted) {
                      showPolygonModal(context, polygon);
...
void dispose() {
  ...
  editorUpdateNotifier.dispose();
  
  // 4. Overlay
  removeInfoCardOverlay();
  
  // 5. Clear collections
  _weatherChipKeys.clear();
...
void handleDrawingTap(LatLng point) {
  if (selectedPolygonIndex != null) {
    ...
    // Simply add the point - NO auto-completion here
    currentPolygon.add(point);
    _saveState();
    
    // Optional: You could add a visual indicator when near completion
...
Future<void> completeCurrentPolygon(BuildContext context) async {
  ...
  try {
    ...
    final tempPolygon = PolygonData(
      ...
      color: Colors.purple  , 
      status: isFarmer ? 'Inactive' : 'Active',
    );

    // Calculate area
    tempPolygon.area = calculateAreaInHectares(tempPolygon.vertices);
    tempPolygon.color = tempPolygon.area! > 5 ? Colors.red : Colors.purple;
